...
$("#help-menu-item").click(function (event) {
    ...
    var helpContent = [
        ...
        { command: "Hold space:", description: "Draw undirected arrows (currently disabled because of bug)" },
        { command: "Backspace:", description: "Delete selected elements" },
        { command: "o:", description: "Create observed variable at mouse location" },
        { command: "l:", description: "Create latent variable at mouse location" },
        { command: "c:", description: "Create constant variable at mouse location" }
    ];
...
$(document).on("focusin", function (event) {
    if ($("#help-modal").is(":visible") && !$(event.target).closest(".modal-content").length) {
...
function generateHelpList(contentArray) {
    ...
    for (var i = 0; i < contentArray.length; i++) {
      ...
      helpList += '<div class="help-item"><strong>' + item.command + '</strong><span>' + item.description + '</span></div>';
    }
    return helpList;
  }
