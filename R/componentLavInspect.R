...
lavInspectServer <- function(id, fit) {
  moduleServer(id, function(input, output, session) {
    ...
    li_misc <- c(
      "", "UGamma", "list", "fit.measures", "modification.indices", "options",
      "call", "timing", "test", "post.check", "zero.cell.tables"
    )
    li_alias <- c(
      "se", "std.err", "start", "est", "x", "std", "std.all", "sampstat",
    ...
    li_all <- c(
      li_modmat, li_infdata[-1], li_obssampstat[-1], li_modfeatures[-1],
