  ...
  if (is.character(object)) {
    if (!file.exists(object)) {
      ...
      if (!"try-error" %in% class(mod)) {
        ...
        stop("Input string neither an existing file or Lavaan model.")
      }
    }
    # Find file:
...
getLVNamesPlot <- function(fit) {
  if (inherits(fit, "fake_lavaan")) {
    fit$latents
  } else {
    lavaanNames(fit, type = "lv")
  }
}
...
setMethod("semPlotModel_S4", signature("lavaan"), function(object) {
  ...
  pars <- myParameterEstimates(object, standardized = TRUE)
  list <- inspect(object, "list")


  # Extract variable and factor names:
  varNames <- getOVNames(fit)
...
semPaths <- function(object, what = "paths", whatLabels, style, layout = "tree", intercepts = TRUE, residuals = TRUE, thresholds = TRUE,
  ...
  if (!missing(groups)) {
    if (is.character(groups)) {
      if (any(grepl("man", groups, ignore.case = TRUE)) & any(grepl("lat", groups, ignore.case = TRUE))) {
        ...
        stop("Character specification of 'groups' must contain 'man','lat' or both")
      }
    }
  ...
  for (gr in Groups[(1:length(Groups)) %in% include])
  {
      ...
      if (layout %in% c("tree", "tree2", "tree3")) {
        inBetween <- function(x) {
          if (Layout[x[1], 2] != Layout[x[2], 2]) {
            ...
            return(sum(Layout[Layout[, 2] == Layout[x[1], 2], 1] > min(Layout[x, 1]) & Layout[Layout[, 2] == Layout[x[1], 2], 1] < max(Layout[x, 1])))
          }
        }
        # Curves:
    ...
    ### WHAT TO PLOT? ###
    if (grepl("path|diagram|mod", what, ignore.case = TRUE)) {} else if (grepl("stand|std", what, ignore.case = TRUE)) {
    ...
    if (!missing(whatLabels)) {
      if (grepl("path|diagram|model|name|label", whatLabels, ignore.case = TRUE)) {
        ...
        stop("Could not detect use of 'whatLabels' argument")
      }
    }
    ...
    if (grepl("lisrel", style, ignore.case = TRUE) & residuals & covAtResiduals) {
      ...
      isResid <- rep(FALSE, nrow(Edgelist))
    }
    ...
    if (any(c("Between", "Within") %in% GroupPars$BetweenWithin)) {
      if (all(GroupPars$BetweenWithin == "Within")) {
        ...
        stop("BetweenWithin not only 'Between' or 'Within'.")
      }
    }
